...
#include "LimenProxyInventory/Public/Components/LimenProxyInventory.h"
...
const FName ALimenPlayerCharacter::ProxyInventoryComponentName = TEXT("ProxyInventoryComponent");
...
ALimenPlayerCharacter::ALimenPlayerCharacter(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	ProxyInventoryComponent = CreateDefaultSubobject<ULimenProxyInventory>(ProxyInventoryComponentName);
...
float ALimenPlayerCharacter::ApplyPointDamage(AController* DamageInstigator, AActor* DamageCauser, const float DamageTaken)
{
	const float CarriedDamage = Super::ApplyPointDamage(DamageInstigator, DamageCauser, DamageTaken);
	
	ULimenHealthAttribute* Health = GetAbilityComponent()->GetAttribute<ULimenHealthAttribute>();
	check(Health != nullptr);
	check(!FMath::IsNegativeOrNegativeZero(CarriedDamage));

	Health->ModifyValueBy(-CarriedDamage);
...
void ALimenPlayerCharacter::FireInput(const FInputActionInstance& Instance)
{
	if (Instance.GetValue().Get<bool>())
	{
		if (!IsHoldingWeapon())
		{
			LIMEN_LOG(LogLimenPlayer, Warning, this, "Attempted to fire but player is not holding a weapon")
			return;
		}
...
void ALimenPlayerCharacter::ReloadInput(const FInputActionInstance& Instance)
{
	if (Instance.GetValue().Get<bool>())
	{
		if (!IsHoldingWeapon())
		{
			LIMEN_LOG(LogLimenPlayer, Warning, this, "Attempted to reload but player is not holding a weapon")
			return;
		}
